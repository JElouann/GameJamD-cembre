...
public class ColliderTresVicieuse : NetworkBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.layer == 7) //Si c'est un player
        {
            ...
            print(other.gameObject.GetComponent<OwnerComponentManager>()._idPlayer);
            DownloadId(other.gameObject.GetComponent<OwnerComponentManager>()._idPlayer);
        }
    }
    ...
    private void AfficheLoiPlayer(ulong _idPlayer)
    {
        ...
        _loiEnCours.text = _playerDownload.GetComponent<InputLoi>()._valueLaws.Value.ToString();
